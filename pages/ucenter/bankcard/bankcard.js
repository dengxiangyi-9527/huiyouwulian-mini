var cardutil = require('../../../utils/cardutil.js');
...
Page({
  ...
  getUserIdCardNumber: function (e) {
    ...
    var temp = cardutil.bankCardAttribution(e.detail.value)
    console.log('temp:'+temp)
    console.log('bankNumber:' + e.detail.value)
    if (temp == Error) {
      temp.bankName = '';
      temp.cardTypeName = '';
    }
  ...
  submitInfos: function () {
    ...
    if (that.data.name.length == 0 || that.data.phoneNumber.length == 0) {
      ...
      util.request(api.CardSave, {
        bankCard:that.data.bankNumber,
        reUserName: that.data.name,
        phone:this.data.phoneNumber,
        bankName:that.data.cardType,
        bankAddress:that.data.bankName
